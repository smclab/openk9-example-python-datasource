...
from extraction.user_extraction import AsyncUserExtraction
...
def execute():
    """Change parameters with yours in the request, and also in Async Task signature"""

    class AsyncTask(threading.Thread):

        def __init__(self, domain_, username_, password_, timestamp_, company_id_, datasource_id_, ingestion_url_):
            super(AsyncTask, self).__init__()
            self.extraction_task = AsyncUserExtraction(domain_, username_, password_, timestamp_, company_id_,
                                                       datasource_id_, ingestion_url_)

        def run(self):
            global status
            status = self.extraction_task.start()
            status = self.extraction_task.join()
